...
def get_or_create_google_user(db: Session, user_info: dict) -> User:
    ...
    if user:
        ...
        new_user = User(id=uuid4(), email=user_info['email'], first_name=user_info.get('given_name', ''),
                        ...
                        auth_provider='google',created_at=datetime.now())
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
...
# Depends(get_current_user) > outputs to models.TokenData to make object of CurrentUser wich is same like TokenData model
