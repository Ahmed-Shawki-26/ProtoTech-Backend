...
from src.app.schemas.pcb import BoardDimensions, PriceQuote, ManufacturingParameters , SolderMaskColor
...
class QuoteGenerator:
    ...
    def _render_images(self) -> Tuple[bytes, bytes]:
        ...
        selected_theme = self._params.pcb_color
        # Map the BaseMaterial enum to the theme key (which is a string)
        if selected_theme == SolderMaskColor.white:
            theme_name = 'White' # Default green for other FR-4 colors
        elif selected_theme == SolderMaskColor.yellow:
            theme_name = 'Yellow'
        elif selected_theme == SolderMaskColor.blue:
            theme_name = 'Blue'
        elif selected_theme == SolderMaskColor.green:
            theme_name = 'default'
        elif selected_theme == SolderMaskColor.red:
            theme_name = 'Red'
        elif selected_theme == SolderMaskColor.black:
            theme_name = 'Black'
        elif selected_theme == SolderMaskColor.purple:
            theme_name = 'Purple'
        else:
            # Fallback to the.
            theme_name = 'default'
            
        print(f"Using render theme: '{theme_name}'")
    ...
    def _calculate_price(self) -> Optional[PriceQuote]:
        ...
        base_price = panel_area_cm2 * price_per_cm2 * self._params.quantity
