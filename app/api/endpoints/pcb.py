...
from app.schemas.pcb import ManufacturingParameters, GerberQuoteResponse
...
async def recalculate_price(
    ...
    try:
        ...
        print(f"An unexpected error occurred: {e}")
        raise HTTPException(status_code=500, detail="An internal server error occurred during recalculation.")
...
async def calculate_local_price(
    ...
    try:
        ...
        # Calculate price using local pricing service
        price_data = LocalPricingService.calculate_local_price(board_dimensions, params)
        
        return JSONResponse(content=price_data)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        print(f"An unexpected error occurred in local price calculation: {e}")
        raise HTTPException(status_code=500, detail="An internal server error occurred during price calculation.")
